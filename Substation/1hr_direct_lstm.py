...
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0, model_save_path='best_1hr_direct_lstm.pth'):
...
def train_direct_lstm(model, train_loader, val_loader, epochs=100, lr=0.0003902, weight_decay=3.076e-06, patience=10, model_save_path='best_1hr_direct_lstm.pth'):
...
def main():
    ...
    HORIZON = 4              # Predict 4 steps ahead (1 hour total)
    ...
    # Concatenate lists to arrays of shape (N, 4)
    ...
    # Calculate overall metrics on the flattened arrays (all 4 steps evaluated equally)
    ...
    # Extract only the final step (t+4) to prevent zigzag plotting over the 1-interval sliding window
    ...
    plot_single_model_forecast(
        ...
        save_path='direct_lstm_1hr_ahead_forecast.png'
    )
