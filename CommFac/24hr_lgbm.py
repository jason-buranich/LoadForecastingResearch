...
def main():
    ...
    print("--- Starting 24-Hour-Ahead LightGBM Pipeline ---")
    ...
    # Instantiate the models and extract the LightGBM instance (mlr, rf, lgbm)
    _, _, lgbm = get_tabular_models(horizon=HORIZON)
    
    print("Fitting LightGBM 24-Hour Predictor (Training 96 models via MultiOutputRegressor)...")
    lgbm.fit(X_train_flat, Y_train_full)
    
    # Predict on the test set; returns shape (N_samples, 96)
    preds_np = lgbm.predict(X_test_flat)
