...
# ==============================================================================
HORIZON = 4             # Predicts 96 steps ahead (24 hours)
...
def objective(trial):
    ...
    patch_len = trial.suggest_categorical("patch_len", [8, 12, 16, 24, 32])
    hidden_dim = trial.suggest_categorical("hidden_dim", [64, 128, 256])
    nheads = trial.suggest_categorical("nheads", [2, 4, 8])
    num_layers = trial.suggest_int("num_layers", 1, 3)
    dropout = trial.suggest_float("dropout", 0.1, 0.5)
    
    # Fixed: Reduced learning rate ceiling to prevent Transformer gradient explosion
    lr = trial.suggest_float("lr", 1e-5, 2e-3, log=True)
    weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-3, log=True)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Keep only the last 50% of the training data to massively speed up tuning
    ...
    # STRICT num_workers=0 to prevent OpenMP container deadlocks
    ...
    for epoch in range(epochs):
        ...
        # Fixed: Failsafe to immediately prune the trial if gradients explode into NaN or Inf
...
def main():
    print("--- Starting Optuna Tuning 24-Hour PatchTST ---")
    ...
    study = optuna.create_study(direction="minimize", pruner=pruner, study_name="1hr_patchtst_opt")
