...
# Import components from the newly configured La Trobe pipeline
...
def flatten_sequences(X_hist, X_fut):
    """
    Scikit-learn models cannot natively process 3D temporal tensors (Batch, Seq, Features).
...
def main():
    ...
    print("--- Starting 1-Hour-Ahead MLR Baseline Pipeline ---")
    ...
    # Scikit-learn doesn't need a distinct validation set for early stopping.
    # Combine Train + Val to maximize the data the Ridge solver can see during fitting.
    ...
    # Flatten the features specifically for the MLR baseline
    ...
    # Instantiate the Ridge regression model wrapped in MultiOutputRegressor
    mlr, _, _ = get_tabular_models(horizon=HORIZON)
    
    print("Fitting Multiple Linear Regression (Ridge) 1-Hour Baseline...")
    mlr.fit(X_train_flat, Y_train_full)
    
    # Predict on the test set; returns shape (N_samples, 4)
    preds_np = mlr.predict(X_test_flat)
    ...
    plot_single_model_forecast(
        ...
        start_idx=0, 
        horizon=96,
        model_name="1-Hour MLR Baseline (t+4 step)", 
