...
def main():
    ...
    print("--- Starting 15-Minute MLR Baseline Pipeline ---")
    ...
    # Flatten the data specifically for the MLR baseline
    ...
    # Instantiate the Ridge regression model from models_2.py
    mlr, _, _ = get_tabular_models(horizon=HORIZON)
    
    print("Fitting Multiple Linear Regression (Ridge) Baseline...")
    mlr.fit(X_train_flat, Y_train_full)
    
    # Predict on the test set
    preds_flat = mlr.predict(X_test_flat)
    ...
    plot_single_model_forecast(
        ...
        start_idx=0, 
        horizon=96,
        model_name="15-Min MLR Baseline", 
