...
# ==============================================================================
class EarlyStopping:
    def __init__(self, patience=15, min_delta=0, model_save_path='best_24hr_transformer.pth'):
...
class GridTransformer(nn.Module):
    def __init__(self, hist_features, fut_features, seq_len=96, horizon=96, d_model=64, n_heads=4, num_layers=2, dropout=0.1):
        ...
        # Scale the output layer mathematically to project a multi-step horizon
    ...
    def forward(self, x_hist, x_fut):
        ...
        # Flatten future covariates from (Batch, Horizon, Features) to (Batch, Horizon * Features)
...
# 2. TRAINING LOOP
# ==============================================================================
def train_transformer(model, train_loader, val_loader, epochs=100, lr=1e-3, weight_decay=1e-4, patience=15, model_save_path='best_24hr_transformer.pth'):
...
# ==============================================================================
def main():
    # 24-Hour (96-Step) Microgrid Configuration
    HORIZON = 96              
    SEQ_LEN = 96             
    TARGET_IDX = 1           
    COVARIATE_START_IDX = 2  
    
    print("--- Starting 24-Hour-Ahead Tuned Grid Transformer Pipeline ---")
    ...
    # Instantiate Model with Tuned Architecture Parameters
    ...
    # Keep the 2D shape (N, 96) for proper metric extraction
    ...
    print("\n--- 24-Hour-Ahead Tuned Grid Transformer Metrics (kW) ---")
    ...
    t96_targets = inverse_scale(targets_np[:, 95])
