...
# Import components from your data pipeline and visualization modules
...
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0, model_save_path='best_24hr_patchtst.pth'):
...
def train_patchtst(model, train_loader, val_loader, epochs=100, lr=0.0005206, weight_decay=0.0009997, patience=10, model_save_path='best_24hr_patchtst.pth'):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch_x_hist, batch_x_fut, batch_y in val_loader:
                ...
                # Forward pass passing both tensors
                outputs = model(batch_x_hist, batch_x_fut)
...
def main():
    ...
    print("--- Starting 24-Hour-Ahead PatchTST Pipeline ---")
    ...
    # 3. DataLoaders (batch_size=32 for memory stability on 96-step sequences)
    ...
    model_path = 'best_24hr_patchtst.pth'
    
    # 4. Instantiate PatchTST Model with Tuned Hyperparameters
    model = PatchTST(
    ...
    with torch.no_grad():
        for batch_x_hist, batch_x_fut, batch_y in test_loader:
            # Map both history and future to GPU
    ...
    print("\n--- 24-Hour-Ahead PatchTST Metrics (MW) ---")
    print(f"RMSE: {rmse:.2f} | MAE: {mae:.2f} | WAPE: {wape:.2f}%")
    
    # Isolate the t+96 interval (index 95) for visualization
    ...
    plot_single_model_forecast(
        ...
        start_idx=0, 
        horizon=96,
        model_name="24-Hour PatchTST (t+96 step)", 
        save_path='patchtst_24hr_ahead_forecast.png'
    )
