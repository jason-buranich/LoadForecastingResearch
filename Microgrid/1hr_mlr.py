...
def main():
    # 1. 1-Hour (4-Step) Microgrid Configuration
    ...
    print("--- Starting 1-Hour-Ahead MLR Baseline ---")
    ...
    # Target shape is (N, 4). MultiOutputRegressor expects exactly this 2D shape.
    Y_train_np = Y_train.numpy()
    Y_test_np = Y_test.numpy()
    
    # 4. Instantiate Model
    # get_tabular_models automatically wraps Ridge in MultiOutputRegressor when horizon > 1
    mlr, _, _ = get_tabular_models(horizon=HORIZON)
    
    # 5. Train
    print("Training MLR (Ridge Regression MultiOutput)...")
    ...
    # Calculate metrics across the entire 4-step horizon
