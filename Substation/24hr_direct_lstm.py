...
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0, model_save_path='best_24hr_direct_lstm.pth'):
...
def train_direct_lstm(model, train_loader, val_loader, epochs=100, lr=1e-3, weight_decay=1e-4, patience=10, model_save_path='best_24hr_direct_lstm.pth'):
    ...
    for epoch in range(epochs):
        ...
        for batch_x_hist, batch_x_fut, batch_y in train_loader:
            ...
            # Direct forward pass passing both tensors (no teacher forcing needed)
...
def main():
    ...
    print("--- Starting 24-Hour-Ahead Direct LSTM Pipeline ---")
    ...
    # 3. DataLoaders (LSTMs can handle larger batch sizes than Transformers)
    train_loader = DataLoader(TensorDataset(X_train_hist, X_train_fut, Y_train), batch_size=128, shuffle=True, num_workers=4)
    val_loader   = DataLoader(TensorDataset(X_val_hist, X_val_fut, Y_val), batch_size=128, shuffle=False, num_workers=4)
    test_loader  = DataLoader(TensorDataset(X_test_hist, X_test_fut, Y_test), batch_size=128, shuffle=False, num_workers=4)
    
    model_path = 'best_24hr_direct_lstm.pth'
    
    # 4. Instantiate Model 
    # (Update hidden_dim and num_layers to match your tuning results later)
    model = DirectLSTM(
    ...
    print("\n--- 24-Hour-Ahead Direct LSTM Metrics (MW) ---")
    ...
    plot_single_model_forecast(
        ...
        start_idx=0, 
        horizon=96,
        model_name="24-Hour Direct LSTM (t+96 step)", 
        save_path='direct_lstm_24hr_ahead_forecast.png'
    )
