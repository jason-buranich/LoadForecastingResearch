...
def main():
    ...
    HORIZON = 96             # Predict 96 steps ahead (96 * 15 minutes = 24 hours)
    ...
    print(f"--- Starting 24-Hour-Ahead MLR (Ridge) Baseline ---")
    ...
    # For a multi-step horizon, Y_train is shape (N, 96). 
    ...
    print("\n--- 24-Hour-Ahead Substation MLR Evaluation Metrics (MW) ---")
    print(f"RMSE: {rmse:.2f} | MAE: {mae:.2f} | WAPE: {wape:.2f}%")
    
    # 7. Visualization
    # Extract only the final horizon step (t+96) from the unflattened array to plot a true chronological day.
    ...
    plot_single_model_forecast(
        ...
        start_idx=0, 
        horizon=96,
        model_name="24-Hour MLR (t+96 step)", 
